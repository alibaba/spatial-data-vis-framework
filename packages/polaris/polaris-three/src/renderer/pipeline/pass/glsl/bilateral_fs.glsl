...
//         vec4 col = texture2D( tex0, centreUV + texCoordOffset ) + texture2D(
//         tex0, centreUV - texCoordOffset ); colOut += gWeights[i] * col;
//     }
...
// double pass高斯模糊
    ...
    // vec2 coord = vec2(clamp(vUv.x + (float(i) - 6.0) * off, 0.0, 1.0),
    // vUv.y);
    ...
    float w = gaussianKernel[i] * smoothstep(5.0, 3.0, abs(sampleZ - centerZ)) *
              clamp(dot(centerNormal,
                        unpackRGBToNormal(texture2D(normalBuffer, coord).rgb)),
                    0.0, 1.0);
    weightAll += w;
