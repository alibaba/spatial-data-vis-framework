...
namespace Ali.Polaris.Projections
{
    public class MercatorProjection : Projection
    {
        ...
        public MercatorProjection(ProjectionProps props) : base(props)
        {
        ...
        {
            return new double[] { lnglat[0], Math.Log(Math.Tan(lnglat[1]) + 1 / Math.Cos(lnglat[1])) };
        }

        protected static double[] unprojRaw(double[] xy)
        {
            return new double[] { xy[0], Math.Atan(Math.Sinh(xy[1])) };
        }
    }
}