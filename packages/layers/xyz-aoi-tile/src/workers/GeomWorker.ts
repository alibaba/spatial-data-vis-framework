...
_self.addEventListener('message', (e) => {
	...
	switch (info.task) {
		...
		// case 'featureToLinePositions': {
		// 	const feature = info.feature
		// 	const projectionDesc = info.projectionDesc
		// 	const projection = createFromDesc(projectionDesc)
		// 	const baseAlt = info.baseAlt
		// 	const linePositions = featureToLinePositions(feature, projection, baseAlt)
		// 	result.data = { linePositions }
		// 	result.transferables = []
		// 	// add transferables
		// 	linePositions?.forEach((typedArray) => {
		// 		if (typedArray.buffer) {
		// 			result.transferables.push(typedArray.buffer)
		// 		}
		// 	})
		// 	break
		// }
...
function triangulateGeoJSON(geojson: any): any {
...
// 带细分的CDT三角化
function CDTGeojsonWithSubdivision(data, subdivision): any {
...
function CDTtriangulate(boundaries, steinerPoints) {
...
function pointInsidePolygon(point, boundaries) {
	if (!boundaries[0][0][0]) {
