...
如果一个脚本的任务是操作一个或多个 layer 实例，则应该挂载到这些 `layer 实例` 上，运行时通过 `event.currentTarget` 获取并操作当前的 layer。

如果一个脚本不操作具体的 layer，则可以挂载到 `app` 上。

> ⚠️ 脚本编写者应注意副作用的范围，如果对 target 以外的对象进行修改，Polaris App 将无法保证正确响应配置变化。
