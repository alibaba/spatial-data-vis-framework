...
Polaris App 是使用 [Polaris.gl 分层空间可视化框架](https://github.com/alibaba/spatial-data-vis-framework) 的最佳实践，它提供了一套简单直白的应用结构、脚手架、模版代码和工具集，来降低 PolarisGL 的使用难度和维护成本。
...
Polaris App 中的 Layer 继承 StandardLayer 基类 :
...
-   Layer 开发者不关心数据的来源，只将关心自己的 prop (参数)，可监听 prop 的变化并作相应，也可以让 PolarisApp 自动重建 Layer 实例来响应变化。
-   在 AppConfig 中，定义数据插槽，并关联到具体的 layer prop 上。
-   PolarisApp 的使用者按照自己的节奏给每个插槽传入数据，不关心用途或影响。
...
function createLayer(props) {
    ...
    layer.element.innerHTML = props.message
    return layer
}
...
setInterval(async () => {
    app.updateDate('LOCAL_DATA_0', await fetchMessage())
}, 1000)
```
...
Polaris App 实例使用事件总线来管理所有内部事件，可以添加监听器来管控 app 的运行，也可以发送/监听自定义事件，来实现高度定制的内外联动、复杂交互等。
...
Polaris App 提供`基于事件的脚本系统`，通过配置来实现定制逻辑，无需修改 Layer 类，无需编译打包。脚本系统设计目的是为配置和搭建环节提供低代码扩展能力。

`脚本`本质上是 **挂载到特定对象上的事件监听器**，目前支持挂载到 layer/stage/app 实例上，被总线事件触发后，在每个所挂载的对象上执行一次。

#### handler 运行上下文

脚本所监听的事件触发后，handler 代码会被包装为一个 `Function 对象` 调用执行。

执行上下文中会提供触发该次执行的事件 `e`，类型参见 [Event 接口](../src//private/event/events.ts)，其中：

-   `e.currentTarget` 是脚本所挂载的实例
    -   若挂载多个实例，则在每个实例上分别执行脚本
    -   `scriptInit 事件` 比较特殊，触发时挂载对象可能还未创建，currentTarget 为 null
-   `e.target` 是发出事件的对象
    -   对于`内置事件`来说，该值总是当前的 PolarisApp 实例
    -   对于`自定义事件`来说，该值是事件发送方调用 `getEventBusAgent` 时提供的对象
...
>     -   `DOM Event`会冒泡，父元素上的监听器可以收到到子元素上触发的事件，因此`currentTarget`可以是`target`的父级
>     -   PolarisApp 的 `BusEvent` 是在 App 内全局广播的，所有监听者能收到所有事件，因此`currentTarget`和`target`没有上下级关系

> ⚠️ 暂时没有提供`全局变量管控`或`外部依赖注入`机制，如果自行解决，请关注后续升级的兼容性。

> 💡 自定义事件的`发出者`在`申请busAgent` 时，应提供一个合理的值，来向`接收者`表明自己的身份。
...
> ⚠️ 脚本编写者应注意 handler 副作用的范围。
> PolarisApp 在配置变化后会自动重建受影响的 Layer 并重新挂载脚本，如果脚本对 target 以外的内容进行操作，或者存在异步的、全局的操作，PolarisApp 将无法保证正确响应配置变化，用户需要重载应用才能得到正确结果。
